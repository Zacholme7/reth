    ...
    S: SparseTrie + Default + Clone,
{
    ...
    pub(super) fn new_with_trie(
        ...
        // Concurrent tasks (engine validation next to payload candidates) must not queue
        // behind one named worker.
        executor.spawn_blocking_named_or_tokio("trie-hashing", move || {
